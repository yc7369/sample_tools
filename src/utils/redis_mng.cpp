...
// Copyright (c) 2020 yangchen All rights reserved.
**************************************************************/
...
RedisPtr RedisMng::GetRedis2(bool master, int db) {
    ...
    connection_opts.connect_timeout =
        std::chrono::milliseconds(redis_config_->connect_timeout);  // Required.
    connection_opts.socket_timeout =
        std::chrono::milliseconds(redis_config_->socket_timeout);   // Required.
...
RedisPtr RedisMng::GetRedis(bool master, int db) {
    ...
    redis = std::make_shared<Redis>(sentinel_,
                                    redis_config_->master_name,
                                    (master ? Role::MASTER : Role::SLAVE),
                                    connection_opts,
                                    pool_opts);
    redisPool->totalsize++;
    redisPool->usesize++;
    ...
    if (!isvalid) {
        ...
        if (sentinel_) {
            ...
            redis = std::make_shared<Redis>(
                sentinel_, redis_config_->master_name, Role::MASTER, connection_opts, pool_opts);
        } else {
            ConnectionOptions connection_opts;
            connection_opts.host = redis_config_->host;  // "172.24.13.83";  // Required.
            connection_opts.port =
                redis_config_->port;  // 16379; // Optional. The default port is 6379.
            connection_opts.password = redis_config_->passwd;  // Optional. No password by default.
            connection_opts.connect_timeout =
                std::chrono::milliseconds(redis_config_->connect_timeout);  // Required.
            connection_opts.socket_timeout =
...
void RedisMngProxy::SetInvalid() {
    isvalid_ = false;
}

RedisPtr RedisMngProxy::operator->() const {
    return ptr_;
}

RedisMngProxy::operator void*() const {
    return ptr_.get();
}
