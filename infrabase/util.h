...
/*******************************************************************
File Name: infrabase/include/util.h
...
#endif
inline AgentItem* firstUnusedMonitorAgentItem(const SDSMonitorItem& mnt) {
    constexpr const int n = sizeof(mnt.tq_items) / sizeof(mnt.tq_items[0]);
    for (int i = 0; i < n; i++) {
        if (mnt.tq_items[i].tm_agent_recv == 0 || mnt.tq_items[i].tm_agent_send == 0 ||
            mnt.tq_items[i].tm_handler_send == 0) {
            return const_cast<AgentItem*>(&(mnt.tq_items[i]));
        }
    }

    //不可能到达这里，到这里就出错了，但是避免上层检查不检查崩溃，还是返回一个
    return const_cast<AgentItem*>(&(mnt.tq_items[0]));
}

std::shared_ptr<std::string> inline get_msg_buf(QtpMsgPtr msg) {
    const size_t total_len = msg->data_len() + msg->opt_len() + sizeof(MSG_HEAD);
    std::string buf;
    buf.resize(total_len);
    const auto head = msg->head();
    memcpy((void*)buf.data(), (const void*)&head, sizeof(MSG_HEAD));
    memcpy((void*)buf.data(), msg->options(), msg->opt_len());
    memcpy((void*)(buf.data() + sizeof(MSG_HEAD) + msg->opt_len()), msg->data(), msg->data_len());
    return std::make_shared<std::string>(std::move(buf));
}
